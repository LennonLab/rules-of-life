...
def immigration(sD, iD, rL, ps, sd=1):
    ...
    for j in range(sd):
        ...
        if p not in sD:
            sD[p] = {'gr' : np.random.uniform(gr, 1)}
            sD[p]['mt'] = np.random.uniform(mt, 1)
...
def iter_procs(iD, sD, rL, ps, ct):
    ...
    for p in procs:
        ...
        elif p == 1: sD, iD = immigration(sD, iD, rL, ps)
...
def run_model(sim):
    print '\n'
    gr = 10**np.random.uniform(-2, 0)
    mt = 10**np.random.uniform(-2, 0)
    q = 10**np.random.uniform(-2, 0)
    r = 10**np.random.uniform(-2, 0)
    ...
    while ct < 300:
        ...
        output(iD, sD, rL, sim, ct, ps)



for sim in range(100000): run_model(sim)
