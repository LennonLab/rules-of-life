...
S2 = float(S_ln)
N = float(avgN)
#ax.text(28, S2*1.05, 'Earth', fontsize=fs, color = 'k')
Ns.append(N)
DomSs.append(S2)
...
label1 = 'Prediction from Mora et al. (2011)'
label2 = 'Prediction from Larsen et al. (2017)'
label3 = 'Lognormal predictions from Locey and Lennon (2016)'
label4 = 'Microbial data: $S$ = 7.6'+'$N^{0.35}$'
label5 = 'IBMs w/ lognormal dynamics: $S$ = '+str(round(10**b,2))+'$N^{'+str(round(m,2))+'}$'

Micy = (np.log10(7.6)+np.arange(32)*0.35).tolist()
Micx = range(32)

l1 = plt.scatter([19.3], [6.939], color = '0.9', s = 100, linewidth=1.5, edgecolor='0.4', label=label1)
l2 = plt.scatter([28.9], [9.24], color = '0.6', s = 50, linewidth=2, edgecolor='0.4', label=label2)
l3 = plt.scatter(Ns, Ss, color = '0.3', s = 50, linewidths=2, edgecolor='k', label=label3)
l4, = plt.plot(Micx, Micy, '--', lw=0.5, color='r', label=label4)
l5, = plt.plot(xp, p(xp), '--', lw=1, color='0.2', label=label5)

# Create a legend for the first line.
first_legend = plt.legend(handles=[l1, l2, l3, l4, l5], loc=2, frameon=False)
...
newL = tdf['Taxon']
newL[7] = 'primates'
colors = ['red', 'orange', 'green', 'blue', 'cyan', 'm', 'lime', 'Steelblue']
newN = tdf['Log10N']
newS = tdf['Log10S']
...
plt.legend(handles=[l4, l5, l6, l7, l8, l9, l10, l11], loc=4, frameon=False, fontsize=fs)

#### Final Format and Save #####################################################
plt.subplots_adjust(wspace=0.4, hspace=0.4)
plt.savefig(mydir + '/results/figures/RoL.png', dpi=200, bbox_inches = "tight")
plt.close()
